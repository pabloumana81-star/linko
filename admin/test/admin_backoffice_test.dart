...
void main() {
  ...
  testWidgets('admin access is allowed', (tester) async {
    await _pumpAdminRoute(tester, admin, AdminRoutes.dashboard);
  ...
  testWidgets('non-admin users are blocked from admin routes', (tester) async {
    await _pumpAdminRoute(tester, regularUser, AdminRoutes.dashboard);
  ...
  testWidgets('non-admin users are blocked from professional detail', (
    tester,
  ) async {
    await _pumpAdminRoute(
      tester,
      regularUser,
      '/professionals/professional-carlos',
    );

    expect(find.text('Acceso restringido'), findsOneWidget);
    expect(
      find.byKey(const ValueKey('admin-professional-detail')),
      findsNothing,
    );
  });
  ...
  testWidgets('desktop navigation loads every admin section', (tester) async {
    ...
    for (final section in AdminSection.values.skip(1)) {
      ...
      expect(adminRouter.state.uri.path, section.path);
    }
  });
